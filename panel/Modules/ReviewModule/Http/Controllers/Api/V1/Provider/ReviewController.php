...
class ReviewController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // $reviews = $this->review->with(['booking.detail', 'provider', 'customer','reviewReply','service'])->where('provider_id', $request->user()->provider->id)
        //     ->when($request->has('status') && $request['status'] != 'all', function ($query) use ($request) {
        //         return $query->ofStatus(($request['status'] == 'active') ? 1 : 0);
        //     })->latest()->paginate($request['limit'], ['*'], 'offset', $request['offset'])->withPath('');
         $reviews = $this->review->with(['booking.detail', 'provider', 'customer','reviewReply','service'])->where('provider_id', $request->user()->provider->id)->get();
