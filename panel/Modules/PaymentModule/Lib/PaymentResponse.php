...
//        }
    ...
    public static function walletRechargeSuccess($data): array|RedirectResponse|string
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $request = collect([
                ...
                'payment_platform' => $additional_data['payment_platform'] ?? null,
                'name' => $additional_data['name'] ?? null,
                'wallet_status' => $additional_data['wallet_status'] ?? null,
                'callback' => $additional_data['callback'] ?? null,
            ]);

            $result = self::handleWalletRecharge( 
                provider: $request['provider_id'],
                request: $request->toArray(),
                price: $request['amount'],
                name: $request['name']
            ...
            if ($result) {
                $payment_request = PaymentRequest::find($payment_request_id);
                $payment_request->attribute = 'provider-recharge';
                $payment_request->attribute_id = $request['provider_id'];
                $payment_request->save();

            }else{
                DB::rollBack();
                return ['error' => 'Wallet Recharge process failed. Please try again.'];
            }
    ...
    public static function walletRechargeFailed($data): array|RedirectResponse|string
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $result = self::handleWalletRechargeFailed( 
                provider: $request['provider_id'],
                request: $request->toArray(),
                price: $request['amount'],
                name: $request['name']
