...
if (!function_exists('wallet_success')) {
     ...
     * @return void
     */
    function wallet_success($data): void
    {
        ...
        if ($walletStatus['wallet_status'] == 'wallet_recharge'){
            PaymentResponse::walletRechargeSuccess($data);
        } 
    } 
}

if (!function_exists('wallet_fail')) {
    /**
     * @param $data
     * @return void
     */
    function wallet_fail($data): void
    {
        $additional_data = json_decode($data['additional_data'], true);
        $walletStatus = collect([
            'wallet_status' => $additional_data['wallet_status'] ?? null,
        ...
        if ($walletStatus['wallet_status'] == 'wallet_recharge') {
            PaymentResponse::walletRechargeFailed($data);
        }
    }
}