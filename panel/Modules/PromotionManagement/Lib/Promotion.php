...
if (!function_exists('gst_calculation')) {
    function gst_calculation($var_id)
    { 
        $variation = Variation::find($var_id);
        
        $con_per =  $variation ? ($variation->contax ? ($variation->contax->cgst + $variation->contax->sgst + $variation->contax->igst) : 0) : 0;
        $agg_per =  $variation ? ($variation->aggtax ? ($variation->aggtax->cgst + $variation->aggtax->sgst + $variation->aggtax->igst) : 0) : 0;
        $con_fee = $variation ? $variation->convenience_fee : 0;
         
        $agg_fee = $variation ? $variation->aggregator_fee : 0;
