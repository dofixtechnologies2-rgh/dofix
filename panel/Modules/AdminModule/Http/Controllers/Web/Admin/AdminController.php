...
class AdminController extends Controller
{
    ...
    public function dashboard(Request $request, Transaction $transaction): View|Factory|Application
    {
        ...
        $commission_earning_new = DB::table('accounts')->sum('account_payable');

        $data = [];
        $data[] = ['top_cards' => [
//            'total_commission_earning' => $commission_earning ?? 0,
            'total_commission_earning' => $commission_earning_new ?? 0,
//            'total_fee_earning' => $fee_amounts ?? 0,
            'total_Services' => DB::table('services')->whereNull('deleted_at')->count(),
        ...
        foreach ($months as $month) {
            $found = 0;
            $monthWiseBookings = DB::table('bookings')
                ->whereBetween('created_at', [
                    date('Y-' . $month . '-01 00:00:00'),
                    date('Y-' . $month . '-31 23:59:59'),
                ])->count();
            $monthWiseBookingsAmount = DB::table('bookings')
                ->whereBetween('created_at', [
                    date('Y-' . $month . '-01 00:00:00'),
                    date('Y-' . $month . '-31 23:59:59'),
                ])->sum('total_booking_amount');
            $chart_data['total_earning'][] = with_decimal_point($monthWiseBookingsAmount);
            $chart_data['commission_earning'][] = with_decimal_point($monthWiseBookings);
