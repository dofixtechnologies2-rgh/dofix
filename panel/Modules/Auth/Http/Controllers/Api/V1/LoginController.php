...
class LoginController extends Controller
{
    ...
    public function __construct(Category $category, User $user, LoginSetup $loginSetup)
    {
        $this->user = $user;
    ...
    public function userDetails(Request $request)
    {
        ...
        $userData->full_profile_path = asset('provider/profile_img/' . $userData->profile_image);

        return response()->json(response_formatter(DEFAULT_200, $userData), 200);
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        User::where('id', auth()->user()->id)->update([
            ...
            'last_name' => $request->last_name,
            'email' => $request->email,
            'profile_image' => $profile_img ?? User::where('id', auth()->user()->id)->first()->profile_image,
        ]);
    ...
    public function checkUser(Request $request): JsonResponse
    {
        ...
        $user = $this->user->where('phone', $request->phone)
            ->ofType(CUSTOMER_USER_TYPES)
            ->first();
        ...
        if ($user) {
            ...
            $responseData['token'] = $user->createToken(CUSTOMER_PANEL_ACCESS)->accessToken;
            $responseData['message'] = 'User found successfully';
            $responseData['profile_completed'] = $user->is_phone_verified && !empty($user->first_name);
        } else {
            $tempUser = new User();
            $tempUser->phone = $request->phone;
            $tempUser->user_type = 'customer';
            $tempUser->is_active = 0;
            $tempUser->save();
            $responseData['token'] = $tempUser->createToken(CUSTOMER_PANEL_ACCESS)->accessToken;
        }

        return response()->json(response_formatter(DEFAULT_200, $responseData), 200);
    }
